...
| `validate.py` | Pre-flight lint: per-page footnote numbering is a gapless 1..k with matching sources; cross-references (`p15`) point to pages that exist; **column discipline — hard-fails any interior `p.body` at full page measure** (must sit in `.cols2`/`.cols3`/`.grid`; escape hatch `class="body fullmeasure"` for a deliberate opener). |
