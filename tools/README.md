...
| `validate.py` | Pre-flight lint: per-page footnote numbering is a gapless 1..k with matching sources; cross-references (`p15`) point to pages that exist; **column discipline — hard-fails any interior `p.body` at full page measure** (must sit in `.cols2`/`.cols3`/`.grid`; escape hatch `class="body fullmeasure"` for a deliberate opener).; **house-style regression floors** (min counts of chatter/sceptic/stat/pull/panels/dropcaps/figframes/brief strips/footnotes, calibrated on No. 38) so an under-furnished book cannot ship. |
